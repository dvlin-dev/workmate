...
> **范围扩展（Skills 开放平台）**：在"周目标管理"核心之上，agent 增设**可插拔技能（Skill）+ 执行工具面**（文件 read/write/edit/ls/glob/grep、bash、web fetch/search，产物落 `userData/workspace`）。技能用 `SKILL.md` 描述、按需 `skill` 工具加载、设置/侧边栏可启停。**本期演示取向：开放最大权限，bash/文件工具无沙箱、无审批**（仅留钩子，未来面向不可信 skill 再补）。详见 `docs/plan/skills-integration.md`。
...
1. **绝不硬编码任何密钥**。`apiKey` 由用户在设置页填写并落本地。baseURL 默认内置OpenAI 兼容服务，但可改。本项目会开源，须保持通用。
2. **无 key 不跑对话 mock**：首次启动或发送消息时若 `apiKey` 为空，直接打开设置页引导用户填写真实 LLM 配置；`MockLanguageModelV3` 仅允许显式用于单测。
...
6. **agent 循环上限**（agents-core `run(..., { maxTurns })`）兜底：轻量归因默认 8；开放任务（skill + 文件/bash 多步产出）放宽到 30。可选 doom-loop 同参去重。
...
| LLM/Agent | `@openai/agents-core` + Vercel AI SDK（`ai` + `@ai-sdk/openai-compatible`）；测试可显式用 `MockLanguageModelV3` |
...
| Agent 运行时（agents-core+AI SDK）、模型构建、Tool（zod）、显式测试 mock | `docs/reference/agent-runtime.md` |
...
| Skills 开放平台（技能 + 执行工具面 + 侧边栏）方案与执行 | `docs/plan/skills-integration.md` |
