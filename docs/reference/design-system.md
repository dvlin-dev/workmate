...
蓝 **`#455DD3`** → 落为 `--brand` token（Light `229 62% 55%` / Dark `229 62% 62%`，另有 `--brand-light`），渐变 `linear-gradient(135deg,#455DD3,#6B7FE0)`。区别于进度条的 `--success` 绿。App 内蓝色只用于关键强调；官网 Hero 可用渐变。**易改**：调 `--brand` 一处即可。详见 [`branding.md`](./branding.md)。
